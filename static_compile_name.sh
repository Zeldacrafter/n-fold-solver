#!/usr/bin/bash
local_opts="-I third-party/hopscotch-map/include -I /usr/include/eigen3 -rdynamic -ldl -lbacktrace -DUSING_BOOST"
compile_flags="-std=c++17 -Ofast -g" # -fsanitize=address" # -fsanitize=undefined"

if [ "$#" -eq 5 ]; then
    echo "Compiling for n = $1, r = $2, s = $3 and t = $4"
    g++ main.cpp -o $5.out $compile_flags $local_opts -DN_NFOLD=$1 -DR_NFOLD=$2 -DS_NFOLD=$3 -DT_NFOLD=$4
elif [ "$#" -eq 2 ]; then
    echo "Looking for params in file $1."
    line=$(head -n 1 $1)
    vals=($line)
    echo "Found n = ${vals[0]}, r = ${vals[1]}, s = ${vals[2]}, t = ${vals[3]}. Starting compilation"
    g++ main.cpp -o $2.out $compile_flags $local_opts -DN_NFOLD=${vals[0]} -DR_NFOLD=${vals[1]} -DS_NFOLD=${vals[2]} -DT_NFOLD=${vals[3]}
else
