...
int main() {
    ...
    vi l(n * t), u(n * t), b(r + n*s), c(n * t);
    vvvi as(n, vvi(r, vi(t))), bs(n, vvi(s, vi(t)));
    cin >> l >> u >> b >> c >> as >> bs;

    cout << "Maximize\n"
         << "    obj: ";
    F0R(i, SZ(c)) {
        cout << (c[i] < 0 ? "-" : "+") << " " << abs(c[i]) << " x" << i + 1 << " \n"[i == SZ(c) - 1];
    }
    ...
    F0R(row, r + n * s) {
        ...
        F0R(col, n * t) {
            ...
            if(row < r) {
                val = as[col/t][row][col%t];
            } else {
                size_t blockRow = (row - r)/s;
                if(col >= blockRow*t && col < (blockRow + 1)*t) {
                    val = bs[blockRow][(row-r)%s][col%t];
                } else {
                    val = 0;
                }
            }

            cout << (val < 0 ? "-" : "+") << " " << abs(val) << " x" << col + 1;
            if(col == n * t - 1)
                cout << " = " << b[row] << endl;
            else
                cout << " ";
